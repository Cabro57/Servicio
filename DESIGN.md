...
### Lock Screen
`PinPanel`, the "counter clock". It is also the login screen at startup. While locked it faces the customer, so it shows no business data (no cash, no debt). The footer sits at the bottom. The clock panel is pinned to the exact center of the screen, independent of the header and footer. It is a standard `listCard` (`$Table.background`, 1px hairline, 15px radius, no shadow), the same card as list and detail pages. The card opens with the business logo (28px) and name in bold +2, centered, over a hairline separator. Below that: the time in Roboto Light +64 with the seconds in muted Light +22 on the same baseline, the full date muted +3, six PIN dots (14px, a hairline ring when empty, a filled `Label.foreground` dot when entered) and one status line. Behind it there are three static, soft lights, with no animation. Their colors come from the accent: the accent itself and two siblings with the hue shifted about 27 degrees each way. Alpha is 20% on light and 32% on dark. The background is rendered once and redrawn only when the size, the theme or the accent changes. There is no input field: any digit key types into the PIN, Backspace deletes, Esc clears, Ctrl+V pastes, and the sixth digit verifies. A wrong PIN never opens a dialog: the dots turn danger and shake once (380ms, decaying), and the status line says what happened. After 5 failures input waits 30 seconds with a live countdown in warning. The footer shows a muted lock line (how many open windows come back after unlock) on the left, and the version and "Çıkış" on the right.

### Setup Screen
`SetupPanel` (first run) uses the lock screen's language: `AmbientLights` behind, a centered 420px `listCard`, and a footer with the version and "Çıkış". The card starts with a bold +6 title and a muted lead sentence. Two sections follow, each opened by a hairline and a `$h3` title: İşletme (name required, optional e-mail, profile photo) and Kilit PIN'i (PIN and repeat side by side). A full-width primary "Kurulumu tamamla" ends the card. Errors show under the field, never as a toast, and Enter submits.
