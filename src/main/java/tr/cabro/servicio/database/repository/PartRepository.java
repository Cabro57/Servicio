...
public interface PartRepository extends SqlObject {
    ...
    default PageResult<Part> searchFilteredPaged(String searchTerm, Map<String, ColumnFilterValue> filters,
        ...
        if (searching) whereClause.append(" AND (p.name LIKE :search OR p.barcode LIKE :search OR p.model_compatibility LIKE :search OR pc.name LIKE :search OR sup.name LIKE :search OR sup.business_name LIKE :search)");
        ...
        java.util.List<Part> items = getHandle().createQuery(listSql).bindMap(params).mapToBean(Part.class).list();
