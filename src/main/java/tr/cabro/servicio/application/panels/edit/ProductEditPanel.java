...
/**
 * {@link PartEditPanel}'in POS ürün kataloğu için bağımsız muadili — {@link Product} alan
 * kümesi bilinçli olarak farklı: marka var, tedarikçi/uyumlu model yok (bkz. V21 migration notu).
 */
public class ProductEditPanel extends AbstractEditPanel<Product> {
    ...
    private void handleBarcode(String barcode) {
        ...
        productService.get(barcode).thenAccept(product -> {
    ...
    protected boolean validateForm() {
        // Barkodsuz ürünler için (fiyat etiketi olmayan market ürünü vb.) dice butonundakiyle
        // aynı üretici kullanılarak otomatik barkod atanır.
        ...
        if (Validator.exceedsMaxLength(name_field.getText(), 255)) {
            showValidationError("Ürün adı en fazla 255 karakter olabilir.");
            name_field.requestFocus();
            return false;
        }
    ...
    protected Product collectFormData(@NonNull Product data) {
        ...
        applyCurrencyPrice(data, purchase_price_field, Product::setPurchaseCurrency, Product::setPurchasePriceOriginal, Product::setPurchasePrice);
        applyCurrencyPrice(data, sale_price_field, Product::setSaleCurrency, Product::setSalePriceOriginal, Product::setSalePrice);
    ...
    private void applyCurrencyPrice(Product data, CurrencyField field,
                                     java.util.function.BiConsumer<Product, String> currencySetter,
                                     java.util.function.BiConsumer<Product, BigDecimal> originalSetter,
                                     java.util.function.BiConsumer<Product, BigDecimal> tryPriceSetter) {
        String code = field.getSelectedCurrency();
    ...
    public void populateFormWith(Product data) {
        ...
        if (st != null) stock_spinner.setValue(st);
        if (ms != null) min_stock_spinner.setValue(ms);
    ...
    protected void initComponent() {
        ...
        barcode_field.putClientProperty(FlatClientProperties.PLACEHOLDER_TEXT, "Ürün barkodunu okutunuz veya girip Enter'a basınız.");

        add(barcode_field, "grow, push, h 40!, wrap, span 2");

        add(new JLabel("Ürün Adı"), "span, split 2");
